...
def get_residual_unit(name, data, num_filter):
    with tf.name_scope(name) as sub_sub_scope:
        ...
        conv1 = tf.layers.conv2d(inputs=bn1, filters=int(num_filter * 0.25), kernel_size=(1, 1),
                                 strides=(1, 1), activation=tf.nn.relu,
                                 use_bias=False, padding='same')
        bn2 = tf.layers.batch_normalization(inputs=conv1, epsilon=1e-5)
        conv2 = tf.layers.conv2d(inputs=bn2, filters=int(num_filter * 0.25), kernel_size=(3, 3),
                                 strides=(1, 1), activation=tf.nn.relu,
                                 use_bias=False, padding='same')
        bn3 = tf.layers.batch_normalization(inputs=conv2, epsilon=1e-5)
        conv3 = tf.layers.conv2d(inputs=bn3, filters=num_filter, kernel_size=(1, 1),
                                 strides=(1, 1), activation=tf.nn.relu,
                                 use_bias=False, padding='same')
        return data + conv3
...
class ValueNet(object):
    def __init__(self, mgame, renew=True):
        self.mgame = mgame
        self.graph = tf.Graph()
        with self.graph.as_default():
            ...
            with tf.name_scope("Actions_Predict") as scope:
                self.p_conv = tf.layers.conv2d(inputs=self.res_units[-1], filters=mes.NET_ACT_LAST_CNN_FILTER_NUM,
                                               kernel_size=(3, 3), activation=tf.nn.relu, use_bias=False,
                                               padding='same')
            ...
            with tf.name_scope("Train") as scope:
                ...
                with tf.name_scope("Loss") as sub_scope:
                    ...
                    self.policy_loss = tf.negative(tf.reduce_mean(
                        tf.reduce_sum(tf.multiply(self.pies, tf.log(tf.clip_by_value(self.p_dense, 1e-8, 1.0))))))
            ...
            if not renew and mes.MODEL_PATH and os.path.exists(mes.MODEL_PATH):
